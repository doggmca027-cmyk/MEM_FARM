-- =============================================================================
-- Meme Farm — split the two progression axes
--   * user_characters.study_level  (NEW) — raised only by Study, powers PvP only
--   * Study  → PvP power only. Never touches current_income_day or level.
--   * Merge  → merge level → BOTH income (GRAM/day) and PvP power.
--   * Power  = base_power * (1 + 0.35*(level-1) + 0.50*study_level)
--             (merge-level + study-level contributions summed off the base)
--   * Income = base_income_day * 1.75^(level-1)          — merge level only
--   * Hardcore merge roll: 1..50 FAIL, 51..100 SUCCESS; on success a sub-roll
--     picks the gain — +1 (80%) · +2 (15%) · +3 (4%) · +4 (1%)
--   * roll_tier_character: seed current_power on insert (was left NULL)
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. study_level column + power backfill
-- -----------------------------------------------------------------------------
alter table public.user_characters
  add column if not exists study_level int not null default 0;

update public.user_characters uc
set current_power = round(
      ct.base_power * (1 + 0.35 * (uc.level - 1) + 0.50 * uc.study_level), 9)
from public.character_templates ct
where ct.id = uc.template_id;

-- -----------------------------------------------------------------------------
-- 2. study_upgrade_character — PvP power only, income & level untouched
-- -----------------------------------------------------------------------------
...
begin
  ...
  select base_power into v_base_pw
  from public.character_templates where id = v_uc.template_id;

  -- fee scales with the study point being bought (0-indexed): 0.05 * 1.5^study_level
  v_fee := round(0.05 * power(1.5, v_uc.study_level), 9);
  ...
  v_power := round(
    ...
    * (1 + 0.35 * (v_uc.level - 1) + 0.50 * (v_uc.study_level + 1)), 9);

  update public.user_characters
  set study_level = study_level + 1,
  ...
  values (p_user_id, 'STUDY_FEE', 'DEBIT', v_fee, 0, 'GRAM',
          jsonb_build_object('user_character_id', p_user_character_id,
                             'to_study_level', v_uc.study_level + 1));
  ...
  new_level          := v_uc.level;                 -- unchanged
  new_income_day     := v_uc.current_income_day;    -- unchanged
  new_power          := v_power;
...
-- -----------------------------------------------------------------------------
-- 3. merge_user_characters — hardcore 50/50 roll, income + power both grow
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- hardcore roll: 50/50 fail/success, then a sub-roll for the level gain
...
-- -----------------------------------------------------------------------------
-- 4. roll_tier_character — seed current_power on insert (was NULL -> 0 power)
-- -----------------------------------------------------------------------------
