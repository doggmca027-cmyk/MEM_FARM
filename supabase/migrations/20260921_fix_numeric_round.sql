-- =============================================================================
-- Meme Farm — fix "function round(double precision, integer) does not exist"
--
-- merge_user_characters and study_upgrade_character computed fees / income with
-- power(<int>, <int>), which resolves to power(double precision, ...) -> double
-- precision, then fed that to round(x, 9) — a signature that only exists for
-- numeric. Every authenticated craft / study upgrade raised 42883 and the
-- client (which swallows RPC errors) just hung on "КРАФТ…".
-- Fix: force the power() base to numeric so the whole expression stays numeric.
-- Bodies are otherwise identical to 20260911 (study) / 20260913 (merge).
-- =============================================================================
...
begin
  ...
  v_fee := round(0.05 * power(1.5::numeric, v_uc.study_level), 9);
...
-- ---------------------------------------------------------------------------
-- merge_user_characters  (15% success / 85% burn)
-- ---------------------------------------------------------------------------
