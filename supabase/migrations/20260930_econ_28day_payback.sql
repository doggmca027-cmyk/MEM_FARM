-- =============================================================================
-- Meme Farm — economy rebalance: ~28-day average payback.
--
-- Target: for a freshly-rolled level-1 card, cost / E[income_per_day] ≈ 28
-- days. Because TIER_COST and every tier's base income both scale by the
-- same 2^(tier-1) multiplier, this ratio is identical across all 6 tiers —
-- one weight change rebalances the whole game.
--
--   old slot weights  60 / 25 /  9 / 4 / 2   ->  E/cost = 0.03365  -> 29.72d
--   new slot weights  57 / 26 /  9 / 4 / 4   ->  E/cost = 0.03565  -> 28.05d
--   (common -3pp, uncommon +1pp, legendary doubled 2%->4%; rare/epic untouched)
--
-- Merge ("craft") success 15% -> 25%. Safe to raise post-lifetime-cap
-- (20260926): a merge — success OR fail — always deletes the material card,
-- so total cap headroom always drops from 2x(2*cost) to 2x(cost) regardless
-- of outcome; raising success% only reduces how many duplicate cards are
-- wasted reaching that already-fixed ceiling, it cannot inflate total
-- extractable value. Level-gain sub-roll (+1/+2/+3/+4 @ 80/14/5/1%) unchanged.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. roll_tier_character — slot odds 57 / 26 / 9 / 4 / 4
-- -----------------------------------------------------------------------------
...
begin
  ...
  v_slot := case
              when v_roll <= 57 then 1
              when v_roll <= 83 then 2
              when v_roll <= 92 then 3
              when v_roll <= 96 then 4
              else 5
            end;
...
-- -----------------------------------------------------------------------------
-- 2. merge_user_characters — 25% success / 75% burn (was 15% / 85%)
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- 25% success / 75% burn, then a sub-roll for the level gain
  v_roll := floor(random() * 100)::int + 1;
  if v_roll <= 75 then
    v_status := 'FAIL';
    v_delta  := 0;
...
-- -----------------------------------------------------------------------------
-- 3. character_templates.drop_weight — housekeeping only (never read by the
--    roll RPC, which hardcodes its own thresholds above) but kept in sync so
--    the seed data doesn't lie about the live odds.
-- -----------------------------------------------------------------------------
update public.character_templates
set drop_weight = (array[57, 26, 9, 4, 4])[card_slot]
